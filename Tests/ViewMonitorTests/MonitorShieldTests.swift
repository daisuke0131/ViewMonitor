...
struct MonitorShieldTests {
    ...
    @Test("UIKit 対象ビュー内の計測ボタンへのタッチは盾を通り抜ける")
    func shieldPassesTouchesToUIKitMonitorButtons() throws {
        // UIKit 対象の計測ボタンは対象ビューの subview として追加されるため、
        // rootView 直下の盾より背面に居る。盾が素朴に全タッチを吸収すると
        // UIKit 要素が一切計測できなくなる。
        let window = makeWindow()
        let label = UILabel(frame: CGRect(x: 16, y: 100, width: 100, height: 20))
        window.addSubview(label)
