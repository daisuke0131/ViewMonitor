...
# Generates ViewMonitorSwiftUIExample.xcodeproj so the project can be
# recreated without going through the Xcode GUI.
#
#   bundle exec ruby Example/ViewMonitorSwiftUIExample/generate_project.rb
#
...
EXAMPLE_DIR = File.join(ROOT, 'Example', 'ViewMonitorSwiftUIExample')
APP_DIR     = File.join(EXAMPLE_DIR, 'ViewMonitorSwiftUIExample')
PROJECT_PATH = File.join(EXAMPLE_DIR, 'ViewMonitorSwiftUIExample.xcodeproj')
TARGET_NAME = 'ViewMonitorSwiftUIExample'
BUNDLE_ID   = 'daisuke.ViewMonitorSwiftUIExample'
UITEST_TARGET_NAME = 'ViewMonitorSwiftUIExampleUITests'
...
group = project.new_group(TARGET_NAME, 'ViewMonitorSwiftUIExample')

sources = %w[ViewMonitorSwiftUIExampleApp.swift ContentView.swift]
...
uitest_sources = %w[ViewMonitorUITests.swift]
...
target.build_configurations.each do |config|
  ...
  # A pure-SwiftUI app needs no storyboard or scene manifest, so the plist is
  # fully generated. Launch-screen generation avoids letterboxed rendering.
  ...
  # There is no asset catalog, so drop the gem's defaults that reference one.
...
# Create a shared scheme so CI can refer to it with -scheme.
